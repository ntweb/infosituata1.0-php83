@php
    ...
    $_enable_private = isset($enable_private) ? $enable_private : false;
@endphp
    ...
    <div class="card-body">
        <div class="tab-content">
            <div class="tab-pane show active" id="tab-upload">
                {{-- Upload standard --}}
                <form class="{{ $class }}" id="frmAjaxForm" action="{{ $action }}" autocomplete="none" method="post" data-callback="getHtml('{{ route('upload.attachments', [$item->id]) }}', '#show-attachments')">
                    @csrf
            ...
            <div class="tab-pane show" id="tab-upload-multi">
                ...
                @component('layouts.components.forms.dropzone', ['action' => $action, 'callback' => "getHtml('". route('upload.attachments', [$item->id]) ."', '#show-attachments');"])
                @endcomponent
            </div>
            <div class="tab-pane show" id="tab-upload-cloud">
                {{-- Upload cloud --}}
                <form class="ns" id="frmUploadCloud" action="{{ $action }}" autocomplete="none" method="post" data-callback="getHtml('{{ route('upload.attachments', [$item->id]) }}', '#show-attachments'); $('#frmUploadCloud')[0].reset();">
                    @csrf
                    <input type="hidden" name="module" value="{{ $module }}">
                    <input type="hidden" name="item_id" value="{{ $item->id }}">
