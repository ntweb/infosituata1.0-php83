@php
    ...
    $placeholder = 'Ricerca anagrafica e tag';

    if (request()->input('_module') == 'squadra') {
        $enableAddButton = false;
        $placeholder = 'Ricerca nome squadra';
    }
...
<form class="{{ $class }}" id="frmCreateNode" action="{{ $action }}" autocomplete="none" method="post" data-callback="refreshTree();">
    @csrf
        ...
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body" id="node-create-body">
                    ...
                    <input type="hidden" name="commessa_item_id" value="">
                    <input type="hidden" name="_parent_id" value="">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="input-group">
                                <input type="hidden" class="searchParameter" name="node" value="{{ $parent->id }}">
                                ...
                                @endif
                                <input type="text" class="form-control searchParameter" name="search" id="textSearchItem" placeholder="{{ $placeholder }}" data-route="{{ $search_route }}">
                                <div class="input-group-append">
                                    ...
                                    @endif
                                    <button class="btn btn-light btnSearchItem" type="button" data-route="{{ $search_route }}"><i class="bx bx-search"></i> Cerca</button>
                                    @if($enableAddButton)
                                        <button class="btn btn-light btnCreateItem" type="button" data-route="{{ route('commessa-node.new-item', [$parent->id, '_module' => request()->input('_module')]) }}"><i class="bx bx-plus"></i></button>
                                    @endif
                                </div>
                            </div>
                        </div>
