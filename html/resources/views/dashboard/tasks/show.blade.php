...
@section('header')
    @component('layouts.components.header', ['subtitle' => 'Analisi', 'icon' => 'bx bx-bar-chart-alt', 'right_component' => 'dashboard.tasks.analisi.components.show-header', 'el' => isset($el) ? $el : null, 'back' => isset($el) ? route('task.edit', $el->id) : null])
        {{ $el->label }}
    @endcomponent
@endsection
    ...
    <div class="mb-3 card">
        <div class="card-header-tab card-header">
            <div class="card-header-title">
                ...
                {{-- $el->label --}}
                @include('dashboard.tasks.contextual-fn-menu.overview')
                @include('dashboard.tasks.contextual-fn-menu.rapportini')
                @include('dashboard.tasks.contextual-fn-menu.checklist')
                @include('dashboard.tasks.contextual-fn-menu.avvisi')
            </div>
            <ul class="nav">
                <li class="nav-item">
                    <a data-toggle="tab"
                       ...
                       href="#tab-overview"
                       data-route="{{ route('task.show', [$el->id, '_refresh' => 'overview-table']) }}"
                       data-route-header-refresh="{{ route('task.show', [$el->id, '_refresh' => 'header']) }}"
                       class="nav-link active show">Overview
                    </a>
                </li>
                <li class="nav-item">
                    <a data-toggle="tab" href="#tab-gantt20" class="nav-link ganttRender20"
                       data-route="{{ route('task.gantt20', [$el->id]) }}">Gantt</a>
                </li>
                <li class="nav-item">
                    <a id="btnRapportini"
                        data-toggle="tab" href="#tab-rapportini" class="nav-link get-html d-flex align-items-center"
                        data-route="{{ route('task-rapportino.index', ['root_id' => $el->id]) }}"
                ...
                <li class="nav-item">
                    <a id="btnChecklist"
                       ...
                       data-route="{{ route('checklist.index', ['reference_id' => $el->id, 'reference_controller' => 'tasks']) }}"
                ...
                <li class="nav-item">
                    <a id="refreshAllegati"
                        ...
                        data-route="{{ route('task.allegati', ['root_id' => $el->id]) }}"
                ...
                <li class="nav-item">
                    <a id="refreshAvvisi"
                       ...
                       data-route="{{ route('task.avvisi', $el->id) }}"
...
{{--                </div>--}}
                <div class="tab-pane show" id="tab-gantt20" role="tabpanel">
                    @include('dashboard.tasks.analisi.gantt20')
                </div>
...
@section('modal')
    ...
    @include('dashboard.tasks.modals.modal-delete-attachment')

    @include('dashboard.tasks.modals.print')

    {{-- Modale conferma associazione costi --}}
    <form class="ns" action="{{ route('task.calculate-costi-consuntivi', $el->id) }}"
    ...
    <form class="ns" id="frmRapportino" action="{{ route('task-rapportino.store') }}"
                        ...
                        <div class="row">
                            <div class="col-lg-8">
                                ...
                                <div class="row">
                                    ...
                                    @component('layouts.components.forms.select2-fasi-task', ['name' => 'commesse_id', 'value' => null, 'class' => 'col-md-8', 'tasks_id' => $el->id])
