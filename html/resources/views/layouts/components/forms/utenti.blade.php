@php
    ...
    $elements = \App\Models\Utente::whereAziendaId($azienda_id)->orderBy('extras1')->orderBy('extras2')->get();
...
<div class="{{ $class }}">
    <div class="position-relative form-group">
        ...
        <select class="multiselect-dropdown form-control {{  $_error_class }}" name="{{ $name }}" id="{{ isset($id) ? $id : $name }}" style="width: 100%">
            ...
            <option value="{{ $el->id }}" @if($el->id == old($name, $value)) selected="selected" @endif>{{ Str::title($el->extras1.' '.$el->extras2) }}</option>
            @endforeach
