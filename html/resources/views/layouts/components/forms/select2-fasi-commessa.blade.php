...
<div class="{{ $class }}">
    <div class="position-relative form-group">
        ...
        <select class="select2-fasi-commessa-ajax form-control {{  $_error_class }}" name="{{ $name }}" id="{{ isset($id) ? $id : $name }}" style="width: 100%" @if($_read_only) disabled readonly @endif data-route="{{ route('commessa.fasi-select2', [$commesse_id]) }}">
            @if($_item)
