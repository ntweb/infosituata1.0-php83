@php
    ...
    $onSelectedItem = isset($onSelectedItem) ? $onSelectedItem : null;
@endphp

<div class="{{ $class }}">

    <div class="position-relative form-group text-autocomplete-form-group">

        <label for="exampleEmail11" class="">{{ $slot }}</label>

        <input  placeholder="{{ $placeholder }}"
                type="text"
                class="form-control text-autocomplete {{  $_error_class }}"
                name="{{ $name }}"
                id="{{ isset($id) ? $id : $name }}"
                value="{{ old($name, $value) }}"
                autocomplete="off"
                maxlength="{{ $maxLength }}" @if($_read_only) disabled readonly @endif
                data-route="{{ $route }}"
                data-data-on-selected-item="{{ $onSelectedItem }}">
