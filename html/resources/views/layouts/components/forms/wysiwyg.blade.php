...
{{--        <input placeholder="{{ $placeholder }}" type="text" class="form-control {{  $_error_class }}" name="{{ $name }}" id="{{ isset($id) ? $id : $name }}" value="{{ old($name, $value) }}" autocomplete="off">--}}
        <textarea class="tinymce summernote-xxx editor-xxx" name="{{ $name }}" id="{{ isset($id) ? $id : $name }}">{{ old($name, $value) }}</textarea>
