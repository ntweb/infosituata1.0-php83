...
<div class="{{ $class }}" >
    <div class="position-relative form-group">
        ...
        <input placeholder="{{ $placeholder }}" type="number" class="form-control {{  $_error_class }}" name="{{ $name }}" id="{{ isset($id) ? $id : $name }}" value="{{ old($name, $value) }}" autocomplete="off" step="{{ $step }}" @if($min || $min == 0) min="{{ $min }}" @endif @if($max) max="{{ $max }}" @endif @if($_read_only) disabled readonly @endif {{ $data }}>
