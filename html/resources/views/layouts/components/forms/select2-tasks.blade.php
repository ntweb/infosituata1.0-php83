@php
    ...
    $_template = null;
    if (isset($value))
        $_template = \App\Models\TaskTemplate::find($value);
...
<div class="{{ $class }}">
    <div class="position-relative form-group">
        ...
        <select class="select2-tasks-ajax form-control {{  $_error_class }}" name="{{ $name }}" id="{{ isset($id) ? $id : $name }}" style="width: 100%" @if($_read_only) disabled readonly @endif data-dropdownParent="{{ $dropdownParent ?? null }}">
            @if($_template)
